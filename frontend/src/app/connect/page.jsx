...
export default function Connect() {
  ...
  return (
    <>
      <main className="w-full min-h-screen flex items-center justify-center p-gutter">
        <div className="flex flex-col w-full items-center justify-center py-6 px-4 relative">
          ...
          <div className="w-full max-w-lg bg-surface-container-lowest rounded-[20px] shadow-xl overflow-hidden relative z-10 transition-all duration-300">
            <div className="flex items-center justify-between px-space-lg pt-space-lg pb-space-sm">
              <Link href="/" aria-label="Close dialog" className="w-9 h-9 rounded-full bg-surface-container hover:bg-surface-variant flex items-center justify-center text-on-surface transition-colors cursor-pointer group">
            ...
            <div className="px-space-lg space-y-space-md">
              <div className="space-y-space-sm">
                ...
                <div className="grid grid-cols-1 gap-space-sm" id="role-selector-container">
                  <div
                    ...
                    className={`role-card relative p-space-md rounded-xl cursor-pointer transition-all duration-200 ${
                      role === 'creator'
                        ? 'bg-primary-fixed/20 ring-2 ring-primary-container'
                    ...
                    <div className="flex items-start gap-space-md">
                      ...
                      <div className="absolute top-4 right-4 text-primary-container" id="icon-creator">
                        <span className="material-symbols-outlined text-[22px]" style={{fontVariationSettings: '"FILL" 1'}}>
                          {role === 'creator' ? 'check_circle' : 'radio_button_unchecked'}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  >
                    <div className="flex items-start gap-space-md">
                      ...
                      <div className="absolute top-4 right-4 text-primary-container" id="icon-provider">
                        <span className="material-symbols-outlined text-[22px]" style={{fontVariationSettings: '"FILL" 1'}}>
                          {role === 'provider' ? 'check_circle' : 'radio_button_unchecked'}
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="space-y-space-sm">
                <button
                  ...
                  disabled={connecting}
                  className="w-full flex items-center justify-center gap-space-sm py-3.5 px-space-lg rounded-full bg-primary-container hover:bg-[#e0484d] text-on-primary font-label-md text-label-md shadow-md hover:shadow-lg active:scale-[0.985] transition-all cursor-pointer disabled:opacity-75"
                  id="monad-connect-btn"
                  type="button"
                >
                  <span className="material-symbols-outlined text-[20px]">account_balance_wallet</span>
                  <span id="btn-text">{connecting ? "Connecting to Monad..." : "Connect Monad Wallet"}</span>
                </button>
                <button
                  onClick={() => handleConnect('social')}
                  disabled={connecting}
                  className="w-full flex items-center justify-center gap-space-sm py-3.5 px-space-lg rounded-full bg-surface-container-lowest hover:bg-surface-container-low text-on-surface font-label-md text-label-md shadow-sm active:scale-[0.985] transition-all cursor-pointer"
                  type="button"
                >
              ...
              <div className="p-space-sm rounded-xl bg-surface-container-low flex items-center justify-between">
                <div className="flex items-center gap-space-sm min-w-0">
                  ...
                  <span className="font-body-sm text-body-sm text-on-secondary-container truncate">Monad Testnet RPC (10143)</span>
                </div>
            ...
            <div className="px-space-lg py-space-md bg-surface-container-low text-center">
              <p className="font-body-sm text-body-sm text-on-secondary-container max-w-xs mx-auto">
                By connecting, you agree to the <Link className="underline hover:text-on-surface transition-colors font-label-sm" href="/protocol">Autonomous Settlement Protocol Terms</Link>.
              </p>
            </div>
          </div>
          ...
          <div className="w-full max-w-lg mt-space-md flex items-center justify-between px-space-sm">
            <div className="flex items-center gap-space-xs text-on-secondary-container">
              ...
              <span className="font-label-sm text-label-sm">EIP-191 Task Attestation Ready</span>
            </div>
            <div className="flex items-center gap-space-xs text-on-secondary-container">
