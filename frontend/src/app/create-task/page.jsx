...
const CANONICAL_TASK = "OUR COLLEGE IS GONNA PLAN A HACK NEXT MONTH AT 13 IN SHIMLA SO WE NEED TO KNOW THE WEATHER IN THAT WEEK";
  ...
  async function dispatch() {
    ...
    try {
      ...
      if (record.error_message) {
        ...
        // Write to sessionStorage so the dashboard picks it up for live polling.
        // Navigate immediately — the task runs in the background; the dashboard polls it.
        try { sessionStorage.setItem('agentproof_active_request_id', record.request_id); } catch {}
        router.push(`/dashboard?request=${encodeURIComponent(record.request_id)}`);
      }
    } catch (e) {
      setError(e.message);
    } finally {
...
{result && result.request_id && !result.error_message && (
  <span className="font-body-sm text-body-sm text-tertiary text-center">
    Task dispatched ({result.request_id.slice(0, 12)}…) — opening dashboard…
  </span>
)}
</div>
...
<div className="flex flex-col gap-space-sm">
<button className="w-full py-4 px-gutter bg-primary-container hover:bg-primary text-on-primary font-label-md text-label-md font-bold rounded-full shadow-md hover:shadow-lg active:scale-[0.985] transition-all flex items-center justify-center gap-space-sm group disabled:opacity-50" id="lockFundsBtn" onClick={dispatch} disabled={submitting || !objective.trim()} type="button">
