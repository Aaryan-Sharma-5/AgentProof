...
export async function runTask(result: TaskResult) {
  ...
  if (!proof.ok) return { settled: false as const };
  ...
  return { settled: receipt.status === "success", hash };
}
...
export async function runWorker(taskId: `0x${string}`, providerUrl: string, spendingLimit: bigint) {
  const agentFlow = new AgentFlow(spendingLimit);

  const response = await agentFlow.requestWithPayment(providerUrl);
  if (response.status !== 200 || !response.data) {
    return { settled: false as const, stage: "provider" as const, reason: response.reason ?? "provider_error" };
  }

  const { records } = response.data as { records: PricingRecord[] };
  const result: TaskResult = { taskId, records };

  const settlement = await runTask(result);
  if (!settlement.settled) {
    return { settled: false as const, stage: "evaluation_or_settlement" as const };
  }

  return { settled: true as const, hash: settlement.hash, spent: agentFlow.getSpent() };
}
